...
// KHÔNG in warning ra trình duyệt (tránh phá JSON)
error_reporting(E_ALL);
ini_set('display_errors', 0);

// Chỉ start session nếu chưa start (tránh warning session_start)
...
try {
    ...
    if ($action === 'delete') {
        ...
        $stmt = $db->prepare("SELECT COUNT(*) FROM orders WHERE payment_method_id = ?");
        ...
        // Xóa
        $stmt = $db->prepare("DELETE FROM payment_methods WHERE payment_method_id = ?");
        $stmt->bind_param('i', $id);
        $stmt->execute();
        $stmt->close();
    ...
    // Không khớp action nào ở trên
    throw new Exception('Unknown action: ' . $action);

} catch (Throwable $e) {
    // Luôn trả JSON, không để PHP in HTML warning ra ngoài
